...
export function computeVillaCapacity(
  ...
  const extras = (fp: number) => {
    ...
    return { b, a, inEmsal: (emsal.hasBasement && emsal.basementInEmsal ? b : 0) + (emsal.hasAttic && emsal.atticInEmsal ? a : 0) };
  };
  ...
  const grossPerUnit = grossPerVilla + basementPerUnit + atticPerUnit;
  ...
  /* ── Alan üretimi ── */
  const saleablePerUnit = (villa.netPerVilla && villa.netPerVilla > 0 ? villa.netPerVilla : grossPerVilla)
